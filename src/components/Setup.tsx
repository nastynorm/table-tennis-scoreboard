...
export default function Setup(props: SetupProps) {
  ...
  const [doubles, setDoubles] = createSignal(props.config.doubles);
  ...
  const saveConfig = (ev: SubmitEvent) => {
    ...
    const player1Partner =
      data.get("player1Partner") ?? props.matchState.player1.partnerName;
    const player2Partner =
      data.get("player2Partner") ?? props.matchState.player2.partnerName;
    const homeTeamName =
    ...
    const doubles = data.get("doubles") === "on" || false;
    const showServer = data.get("showServer") === "on" || false;
    const nextConfig = {
      matchLength,
      winningScore,
      switchSides,
      doubles,
      showServer,
      matchType: props.config.matchType,
    ...
    props.setMatchState((state) => ({
      ...
      player1: {
        ...
        partnerName: player1Partner as string,
      },
      player2: {
        ...state.player2,
        name: player2Name as string,
        partnerName: player2Partner as string,
      },
    ...
    localStorage.setItem("player1Partner", player1Partner as string);
    localStorage.setItem("player2Partner", player2Partner as string);
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
            <div class="flex flex-col">
              <label
                ...
                class="font-normal tracking-wider text-black font-sports"
              >
                Player 1 Partner (Home)
              </label>
              <div class="border-2 border-transparent focus-within:border-black">
                <input
                  type="text"
                  name="player1Partner"
                  value={props.matchState.player1.partnerName}
                  data-testid="player1-partner-input"
                  id="player1Partner"
                  class="py-2 px-4 w-full font-mono bg-white border-2 border-black focus:outline-none"
                />
              </div>
            ...
            <div class="flex flex-col">
              <label
                ...
                class="font-normal tracking-wider text-black font-sports"
              >
                Player 2 Partner (Visitor)
              </label>
              <div class="border-2 border-transparent focus-within:border-black">
                <input
                  type="text"
                  name="player2Partner"
                  value={props.matchState.player2.partnerName}
                  data-testid="player2-partner-input"
                  id="player2Partner"
                  class="py-2 px-4 w-full font-mono bg-white border-2 border-black focus:outline-none"
                />
              </div>
      ...
      <section class="grid sm:grid-cols-2 mt-4 gap-4">
        ...
        <div class="flex flex-col">
          ...
          <div class="flex items-center mt-2">
            <label
              for="doubles"
              class="flex gap-4 items-center text-xl font-normal tracking-wider font-sports"
            >
              ...
              <input
                ...
                name="doubles"
                class="mr-2 w-5 h-5 text-black border-2 border-black"
                data-testid="doubles-input"
                checked={props.config.doubles}
                onChange={(e) => setDoubles(e.currentTarget.checked)}
              />
            </label>
          </div>
          <div class="flex items-center mt-3">
            ...
            >
              ...
              <input
                ...
                name="showServer"
                class="mr-2 w-5 h-5 text-black border-2 border-black"
                data-testid="show-server-input"
                checked={props.config.showServer}
              />
            </label>
          </div>
          <details class="mt-2">
            ...
            <p class="font-mono">
              Doubles adds a partner name to each side and tracks which team-mate
              is serving. The serve indicator (a ball) shows whose serve it is and
              rotates automatically every 2 points (every point at deuce). Tap a
              ball to set the first server.
            </p>
          </details>
        </div>
